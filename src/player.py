...
class Player(pygame.sprite.Sprite):
    ...
    def shoot(self, bullet_group):
        ...
        if cur_time - self.last_shot > self.shoot_cooldown and self.magazine > 0:
            ...
            if self.is_npc:
                bullet = Bullet(
                    self.rect.centerx + self.rect.width // 2 * dir + 10 * dir,
                    self.rect.centery,
                    self.direction,
                    "bullet3",
                    scale=0.03,
    ...
    def throw(self, grenade_group):
        ...
        if cur_time - self.last_throw > self.throw_cooldown and self.grenades > 0:
            ...
            dir = 1 if self.direction == 0 else -1
            nade = Grenade(
                self.rect.centerx + self.rect.size[0] * 0.1 * dir,
                self.rect.centery,
                self.direction,
            )
    ...
    def finish_reload(self):
        ...
        ammo_needed = (
            MAX_MAGAZINE_CAPACITY - self.magazine
        )  # Calculate how much more ammo can be added to the magazine
        ...
        if self.ammo < ammo_needed:
            ...
            self.ammo -= (
                ammo_needed  # Reduce the ammo by the amount used to fill the magazine
            )
    ...
    def npc_patrol(self, tiles_rect):
        ...
        foot_rect = pygame.Rect(
            self.rect.x
            + (
                self.rect.width if self.patrol_direction == 1 else 0
            ),  # Add width only when moving right
        ...
        if self.count_move >= 3 * TILE_SIZE:
            ...
            if random.random() < 0.2:  # 20% chance to idle
                ...
                self.idle_duration = random.randint(
                    1000, 3000
                )  # Idle between 1 to 3 seconds
    ...
    def npc_vision_and_shoot(self, player, bullet_group, tiles_rect):
        if self.alive and player.alive:
            ...
            if (
                abs(dx) <= VISION_RANGE and abs(dy) <= VERTICAL_TOLERANCE
