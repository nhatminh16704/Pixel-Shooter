...
class Grenade(pygame.sprite.Sprite):
  def __init__(self, x, y, direction, scale=1):
    ...
    self.speed_x = 5 * (1 if direction == 0 else -1)  # Horizontal speed based on direction
    self.speed_y = -12  # Initial upward speed for a parabolic arc
    self.gravity = 1  # Gravity affecting the grenade
    self.friction = 0.8  # Friction applied to slow down horizontal movement on ground
    self.bounce = 0.6  # How much the grenade should bounce
    self.timer = 3000  # Time in milliseconds before the grenade explodes (e.g., 3 seconds)
    self.creation_time = pygame.time.get_ticks()  # Save the time when the grenade is created
    ...
    for tile in tiles_rect:
      if tile[1].colliderect(self.rect):
        ...
        if dy > 0:  # Falling down, so adjust to the top of the tile
          ...
          self.speed_y *= -self.bounce  # Reverse Y direction for a bounce and reduce speed
          self.speed_x *= self.friction  # Apply friction to slow down horizontal movement
  ...
  def explode(self, explosion_group, enemy_group, player):
    ...
    explosion = Explosion(self.rect.centerx, self.rect.centery, scale=1)  # Adjust the scale as needed
    ...
    if distance_to_player <= 2 * TILE_SIZE:  # Check if player is within explosion radius
